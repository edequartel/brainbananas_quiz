...
function brainbananas_cleanup_old_sessions(int $maxAgeHours = 6): void
{
    ...
    $commentSessionsResult = supabase_request(
        'GET',
        'brainbananas_comment_sessions?created_at=lt.' . urlencode($threshold) . '&select=code'
    );

    if (!$commentSessionsResult['ok'] || empty($commentSessionsResult['data'])) {
        return;
    }

    foreach ($commentSessionsResult['data'] as $session) {
        $code = strtoupper(trim((string)($session['code'] ?? '')));
        ...
        supabase_request(
            'DELETE',
            'brainbananas_comment_sessions?code=eq.' . urlencode($code)
        );
    }
}
